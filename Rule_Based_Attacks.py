...
train_dataset_5 = HAR_Dataset(root_dir=DATASET, activity=[1,4,5], subject=[39,40,41,44,56], repetition=[1,2],
                            transform=transforms.Compose([ToTensor()]))
train_loader_5 = DataLoader(train_dataset_5, batch_size=32, shuffle=False, num_workers=2)

test_dataset_5 = HAR_Dataset(root_dir=DATASET, activity=[1,4,5], subject=[39,40,41,44,56], repetition=[3],
                           transform=transforms.Compose([ToTensor()]))
test_loader_5 = DataLoader(test_dataset_5, batch_size=32, shuffle=False, num_workers=2)


#10 Subject Selection

DATASET = '019 West Cumbria Dataset'
train_dataset_10 = HAR_Dataset(root_dir=DATASET, activity=[1,4,5], subject=[39,40,41,44,47,49,50,51,53,56], repetition=[1,2],
                            transform=transforms.Compose([ToTensor()]))
train_loader_10 = DataLoader(train_dataset_10, batch_size=32, shuffle=False, num_workers=2)


test_dataset_10 = HAR_Dataset(root_dir=DATASET, activity=[1,4,5], subject=[39,40,41,44,47,49,50,51,53,56], repetition=[3],
                           transform=transforms.Compose([ToTensor()]))
test_loader_10 = DataLoader(test_dataset_10, batch_size=32, shuffle=False, num_workers=2)
...
def black_box_attack(target_model, x_train, y_train, x_test, y_test, attack_train_size, attack_test_size):
  ...
  mlp_attack = MembershipInferenceBlackBoxRuleBased(target_model)
  ...
  print(f"Members Accuracy: {mlp_train_acc:.4f}")
  print(f"Non Members Accuracy {mlp_test_acc:.4f}")
  print(f"Attack Accuracy {mlp_acc:.4f}")

  precision, recall = calc_precision_recall(np.concatenate((mlp_inferred_train, mlp_inferred_test)),
...
#x_train_5 = np.array([x['mD'] for x in train_dataset_5])
#y_train_5 = np.array([y['activity'] for y in train_dataset_5])
#x_test_5 = np.array([x['mD'] for x in test_dataset_5])
#y_test_5 = np.array([y['activity'] for y in test_dataset_5])
...
x_train_10 = np.array([x['mD'] for x in train_dataset_10])
y_train_10 = np.array([y['activity'] for y in train_dataset_10])
x_test_10 = np.array([x['mD'] for x in test_dataset_10])
y_test_10 = np.array([y['activity'] for y in test_dataset_10])

x_test_10 = np.expand_dims(x_test_10, axis=1)
x_train_10 = np.expand_dims(x_train_10, axis=1)

#attack_train_ratio = 0.5
#attack_train_size_10 = int(len(x_train_10) * attack_train_ratio)
#attack_test_size_10 = int(len(x_test_10) * attack_train_ratio)

attack_train_size_10 = 25
attack_test_size_10 = 25

# Ten Subject

from art.estimators.classification.pytorch import PyTorchClassifier

#PATH5N = 'model_mask_5.pt'

#PATH5N = 'Grad_IDP/Five/GradIA_noise_activity_model_5.pt'
#PATH5N =  'Sal_DP/Five/SalA_noise_activity_model_5.pt'
#PATH5N= 'Grad_DP/Five/GradA_noise_activity_model_5.pt'

#model5N = MultiTaskResNet(5, 3, return_type='activity')
#model5N.load_state_dict(torch.load(PATH5N, map_location=torch.device('cpu')))
#model5N.eval()

#PATH10N = 'model_mask_10.pt'
#PATH10N = 'multi_task_10.pt'
#PATH10N = 'IG_IDP/Ten/GradIA_noise_activity_model_10.pt'
#PATH10N = 'IG_DP/Ten/IGA_noise_activity_model_10.pt'
#PATH10N =  'Sal_DP/Ten/SalA_noise_activity_model_10.pt'
#PATH10N = 'Grad_DP/Ten/GradA_noise_activity_model_10.pt'
#PATH10N = 'ISG_DP/Ten/GradSIA_noise_activity_model_10.pt'
PATH10N = 'IDG_DP/Ten/GradIDA_noise_activity_model_10.pt'
...
mlp_art_model_10N = PyTorchClassifier(model=model10N, loss=nn.CrossEntropyLoss(), input_shape=(1,800,481), nb_classes=3)

test_pred = np.array([np.argmax(arr) for arr in mlp_art_model_10N.predict(x_test_10.astype(np.float32))])
print('Noise model Five Test accuracy: ', np.sum(test_pred == y_test_10) / len(y_test_10))

black_box_attack(mlp_art_model_10N, x_train_10, y_train_10, x_test_10, y_test_10, attack_train_size_10, attack_test_size_10)
